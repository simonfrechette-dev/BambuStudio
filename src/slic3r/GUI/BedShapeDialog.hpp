...
struct BedShape
{
    ...
    bool     is_custom() {
        return m_build_volume.type() == BuildVolume::Type::Convex
            || m_build_volume.type() == BuildVolume::Type::Custom;
    }

    static void    append_option_line(ConfigOptionsGroupShp optgroup, Parameter param, bool can_edit);
    static QString get_name(PageType type);

    PageType   get_page_type();
    QString    get_full_name_with_params();
...
class BedShapePanel : public QWidget
{
    static const std::string NONE;
    static const std::string EMPTY_STRING;

    Bed_2D*            m_canvas{nullptr};
...
public:
    BedShapePanel(QWidget* parent = nullptr)
        : QWidget(parent), m_custom_texture(NONE), m_custom_model(NONE) {}
...
private:
    ConfigOptionsGroupShp init_shape_options_page(const QString& title);
    void    activate_options_page(ConfigOptionsGroupShp options_group);
    QWidget* init_texture_panel();
    QWidget* init_model_panel();
    ...
    QTabWidget* m_shape_options_book{nullptr};
...
{
    BedShapePanel* m_panel{nullptr};

public:
    BedShapeDialog(QWidget* parent = nullptr) : DPIDialog(parent) {}
