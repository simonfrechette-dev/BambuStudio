...
protected:
    ...
    std::unordered_map<QString, QImage> m_hms_local_images; // key-> image name
...
public:
    ...
    // query
    QString  query_hms_msg(const MachineObject* obj, const std::string& long_error_code);
    QString  query_hms_msg(const std::string& dev_id, const std::string& long_error_code);

    bool     is_internal_error(const MachineObject *obj, int print_error);
    QString  query_print_error_msg(const MachineObject* obj, int print_error);
    QString  query_print_error_msg(const std::string& dev_id, int print_error);
    QString  query_print_image_action(const MachineObject* obj, int print_error, std::vector<int>& button_action);

    // query local images
    QImage   query_image_from_local(const QString& image_name);
...
private:
    ...
    QString  _query_hms_msg(const string& dev_id_type, const string& long_error_code, const string& lang_code = std::string("en"));

    bool     _is_internal_error(const string &dev_id_type, const string &long_error_code, const string &lang_code = std::string("en"));
    QString  _query_error_msg(const string& dev_id_type, const std::string& long_error_code, const std::string& lang_code = std::string("en"));
    QString  _query_error_image_action(const string& dev_id_type, const std::string& long_error_code, std::vector<int>& button_action);
};
